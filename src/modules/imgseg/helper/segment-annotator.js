 ...
 */
 ...
 function Annotator(imageURL, options) {
   ...
   this.highlightAlpha =
     options.highlightAlpha || Math.min(255, this.visualizationAlpha + 128);
   ...
   function updateIfActive(event) {
     ...
     if (mousestate.down) {
       if (
         ...
         if (annotator.mode === "polygon") annotator._emptyPolygonPoints();
         //reset
 ...
 Annotator.prototype._initializeVisualizationLayer = function() {
   ...
   layer.resize(this.width, this.height);
   var initialColor = this.colormap[this.defaultLabel].concat([
     this.visualizationAlpha,
   ]);
   ...
   var container = this.container,
     ...
     x = Math.round(
       (event.pageX - offsetLeft + container.scrollLeft) *
         (container.offsetWidth / (container.scrollWidth*this.currentZoom))
     ),
     y = Math.round(
       (event.pageY - offsetTop + container.scrollTop) *
         (container.offsetHeight / (container.scrollHeight*this.currentZoom))
     );
 ...
 Annotator.prototype._addPolygonPoint = function(event) {
   ...
   if (this.polygonPoints.length === 0) {
     ...
     annotator.prevAnnotationImg = ctx.getImageData(
       0,
       0,
       canvas.width,
       canvas.height
 ...
 Annotator.prototype._checkLineIntersection = function() {
   ...
   for (let i = 1; i < this.polygonPoints.length - 2; ++i) {
     var line1StartX = this.polygonPoints[i - 1][0],
       ...
       numerator1 =
         (newLineEndX - newLineStartX) * a - (newLineEndY - newLineStartY) * b,
       numerator2 =
 ...
 Annotator.prototype._updateAnnotation = function(pixels, labels) {
   ...
   labels =
     typeof labels === "object"
       ? labels
       : _fillArray(new Int32Array(pixels.length), labels);
